...
# 阶梯有4种高度
DICT_HIGH = {0.04: [0.36, 0.20], 0.05: [0.4, 0.25], 0.06: [0.44, 0.29], 0.07: [0.48, 0.33],
             0.08: [0.52, 0.37]}
STEP_TRANSLATION = {0.04: [0, 0.015, 0.035, 0.055],
...
motion_agent = TD3(lr_actor=0.00005,
                   ...
                   # load_models=False,
                   load_models=True,
                   save_dir='./models/saved/TD3/2850')

select_agent2 = agent(load_models=True)
score_history = []
...
for i in range(1, 901):
    ...
    if env.new_final_distance[0] <= env.findThreshold:
        print("======== + Solved + ========", end=' ')
        time_dict[env.seed_step].append(env.steps)
    ...
    js = json.dumps(time_dict)
    file = open('time_dict.txt', 'w')
    file.write(js)
    file.close()
